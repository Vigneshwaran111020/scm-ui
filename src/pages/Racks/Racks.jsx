...
export default function Racks({ tabId }) {
  ...
  const handleRefresh = async () => {
    ...
    try {
      const response = await apiFetch('/api/racks');
  ...
  const handleAdd = () => {
    navigate('/wms/racks/add');
  };

  const handleAction = (type, row) => {
    if (type === 'EDIT') {
      navigate(`/wms/racks/${row.id}/edit`, { state: { row } });
    } else {
      navigate(`/wms/racks/${row.id}/view`, { state: { row } });
    }
  };
  ...
  return (
    <CustomTableContainer
      ...
      icon={Database}
      data={data}
      emptyMessage={isLoading ? "Loading racks..." : "No racks found."}
