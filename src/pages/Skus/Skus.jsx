...
export default function Skus({ tabId }) {
  ...
  const handleRefresh = async () => {
    ...
    try {
      const response = await apiFetch('/api/skus');
  ...
  const handleAdd = () => {
    navigate('/wms/skus/add');
  };

  const handleAction = (type, row) => {
    if (type === 'EDIT') {
      navigate(`/wms/skus/${row.id}/edit`, { state: { row } });
    } else {
      navigate(`/wms/skus/${row.id}/view`, { state: { row } });
    }
  };
  ...
  return (
    <CustomTableContainer
      ...
      icon={Database}
      data={data}
      emptyMessage={isLoading ? "Loading skus..." : "No skus found."}
