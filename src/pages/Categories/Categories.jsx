...
export default function Categories({ tabId }) {
  ...
  const handleAdd = () => {
    navigate('/wms/categories/add');
  };

  const handleAction = (type, row) => {
    if (type === 'EDIT') {
      navigate(`/wms/categories/${row.id}/edit`, { state: { row } });
    } else {
      navigate(`/wms/categories/${row.id}/view`, { state: { row } });
    }
  };
