...
export default function Warehouses({ tabId }) {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      // The API returns PageResponse, so the actual array is in response.data.content
  ...
  const handleAdd = () => {
    navigate('/wms/warehouses/add');
  };

  const handleAction = (type, row) => {
    if (type === 'EDIT') {
      navigate(`/wms/warehouses/${row.id}/edit`, { state: { row } });
    } else {
      navigate(`/wms/warehouses/${row.id}/view`, { state: { row } });
    }
  };
  ...
  return (
    <CustomTableContainer
      ...
      icon={Database}
      data={data}
      emptyMessage={isLoading ? "Loading warehouses..." : "No warehouses found."}
